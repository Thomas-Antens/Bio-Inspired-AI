...
def make_figures(env: SoaringEnv, circ, glide):
    ...
    env.reset(seed=SEED)
    gx_f = np.linspace(0, 5500, 200)
    gy_f = np.linspace(0, 4000, 150)
    ...
    ax_full.plot(5250, 2000, "m*", ms=13, zorder=6, label="Goal")
    ax_full.annotate("", xy=(800, 2500), xytext=(1500, 2500),
                     arrowprops=dict(arrowstyle="->", color="k", lw=2))
    ax_full.text(1100, 2600, "Wind 5 m/s\n(thermals drift west)",
                 ha="center", fontsize=8)
    ax_full.set_xlim(0, 5500); ax_full.set_ylim(0, 4000)
